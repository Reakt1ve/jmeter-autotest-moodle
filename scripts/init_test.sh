...
if ping -c 6 ${HOST} -i 0.5 >/dev/null; then
	echo "Сервер доступен в сети"
else
	print_error 'Сервер недоступен. Проверьте соединение'
fi
...
bash ${JMETER_DIR_PATH}/bin/jmeter -n -t ${JMETER_TEST_PLAN_PATH} -Jhost=${HOST} -Jcourseid=${test_course_id} -Jusers=${current_users} -Jusers_path=${USERS_CSV_FILE_PATH} -f -l results.jtl
